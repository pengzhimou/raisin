...
for i in range(pred_days):
    input_data = current_data[-time_step:].reshape(1, time_step, 6)  # 输入包括所有6列数据
    pred = model.predict(input_data)
    predictions.append(pred[0, 0])
    current_data = np.append(current_data, np.array([[current_data[-1, 0], current_data[-1, 1], current_data[-1, 2], pred[0, 0], pred[0, 0], pred[0, 0]]]).reshape(1, 6), axis=0)

# 将预测结果反归一化
predictions = np.array(predictions).reshape(-1, 1)
predictions = np.concatenate((np.zeros((len(predictions), 3)), predictions, predictions, predictions), axis=1)
